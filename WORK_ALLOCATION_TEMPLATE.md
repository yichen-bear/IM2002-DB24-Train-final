...
|------|--------------|---------------------|-------|
| **Task 1** — Relational schema design (`schema.sql`) | 熊浥辰、呂怡柔 |邱鈺婷|熊浥辰負責設計文字版schema，呂怡柔負責將文字轉為實際程式，邱鈺婷改善正規化與設計FK約束|
...
| Section 2 — Normalisation Justification |邱鈺婷| | |
| Section 3 — Graph Database Design Rationale |呂怡柔| | |
| Section 4 — Vector / RAG Design |呂怡柔| | |
| Section 5 — AI Tool Usage Evidence |熊浥辰| | |
| Section 6 — Reflection & Trade-offs |熊浥辰| | |
...
|--------|-----------|---------------------|
| 熊浥辰 | 33.3% | 主要負責`skeleton/seed_neo4j.py`、`skeleton/verify_neo4j.py`(輔助驗證`skeleton/seed_neo4j.py`)、`skeleton/ui.py`，協助`skeleton/agent.py`、`relational/queries.py`、`relational/schema.sql`|
| 邱鈺婷 | 33.3% | 主要負責`skeleton/seed_postgres.py`、`graph/queries.py`，協助`skeleton/agent.py`、`relational/queries.py`、`relational/schema.sql`|
