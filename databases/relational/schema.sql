...
CREATE TABLE IF NOT EXISTS user_credentials (
    user_id            VARCHAR(32) PRIMARY KEY,
    password_hash      TEXT NOT NULL,-- Will store safe hashes generated by Argon2id in the application layer
...
-- Description: Fully normalized junction table resolving the M:N relationship 
--              between National Rail schedules and stations. Enforces strict 
--              1NF atomicity and preserves sequential stop order.
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS national_rail_schedule_stops (
    schedule_id        VARCHAR(32) NOT NULL,
    station_id         VARCHAR(32) NOT NULL,
    stop_order         INT NOT NULL,          -- The sequence number of the stop (1=Origin, 2=Next...)
    travel_time_offset INT NOT NULL DEFAULT 0, -- Minutes from departure time to this stop
    is_pass_through    BOOLEAN DEFAULT FALSE,  -- True if the train passes without stopping

    PRIMARY KEY (schedule_id, stop_order),    -- Enforces sequence uniqueness per train
    ...
    CONSTRAINT chk_rail_stop_order CHECK (stop_order > 0),
    CONSTRAINT chk_rail_offset CHECK (travel_time_offset >= 0)
);

-- ------------------------------------------------------------
-- Table: metro_schedule_stops
-- Description: Normalized junction table mapping urban Metro route sequences,
--              tracking strict stop order and offset timings per line run.
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS metro_schedule_stops (
    metro_schedule_id VARCHAR(32) NOT NULL,
    station_id        VARCHAR(32) NOT NULL,
    stop_order        INT NOT NULL,
    travel_time_offset INT NOT NULL DEFAULT 0,

    PRIMARY KEY (metro_schedule_id, stop_order),
    ...
    CONSTRAINT chk_metro_stop_order CHECK (stop_order > 0),
    CONSTRAINT chk_metro_offset CHECK (travel_time_offset >= 0)
);
